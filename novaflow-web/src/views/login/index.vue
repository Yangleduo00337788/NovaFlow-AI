<template>
  <div class="auth-page">
    ...
    <div class="auth-card auth-card--login" data-testid="login-card">
      <div v-if="copy.showSsoTab" class="auth-tabs">
        <button
          type="button"
          class="auth-tab"
      ...
      <div class="auth-brand">
        ...
        <p class="subtitle">{{ copy.formSubtitle }}</p>
      </div>

      <template v-if="!copy.showSsoTab || activeTab === 'password'">
        ...
        <div v-if="copy.showSsoTab" class="auth-social-divider">其他登录方式</div>

        <div v-if="copy.showSsoTab" class="auth-social-buttons">
          <button
...
import { canAccessRoute, resolvePostLoginPath } from '@/config/access'
...
const copy = {
  formTitle: '欢迎回来 👋',
  formSubtitle: '登录 NovaFlow AI 平台',
  showSsoTab: true,
}
...
async function onSubmit() {
  ...
  try {
    ...
    router.push(
      resolvePostLoginPath(auth.roleCode, redirect, (path) =>
        canAccessRoute(path, {
