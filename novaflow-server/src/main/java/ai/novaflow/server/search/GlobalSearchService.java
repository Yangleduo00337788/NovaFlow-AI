...
public class GlobalSearchService {
    ...
    public List<GlobalSearchItemVO> search(String keyword, int limit) {
        ...
        items.addAll(searchApplications(tenantId, q, perType));
        items.addAll(searchAgents(tenantId, q, perType));
        items.addAll(searchKnowledge(tenantId, q, perType));
        items.addAll(searchWorkflows(tenantId, q, perType));
    ...
    private List<GlobalSearchItemVO> searchApplications(Long tenantId, String keyword, int limit) {
        return applicationMapper.selectListByQuery(
                QueryWrapper.create()
                        ...
                        .limit(limit)
        ).stream().map(this::toApplicationItem).toList();
    }

    private List<GlobalSearchItemVO> searchAgents(Long tenantId, String keyword, int limit) {
        return agentMapper.selectListByQuery(
                QueryWrapper.create()
                        ...
                        .orderBy("updated_at", false)
                        .limit(limit)
        ).stream().map(this::toAgentItem).toList();
    }

    private List<GlobalSearchItemVO> searchKnowledge(Long tenantId, String keyword, int limit) {
        return knowledgeBaseMapper.selectListByQuery(
                QueryWrapper.create()
                        ...
                        .orderBy("updated_at", false)
                        .limit(limit)
        ).stream().map(this::toKnowledgeItem).toList();
    }

    private List<GlobalSearchItemVO> searchWorkflows(Long tenantId, String keyword, int limit) {
        return workflowMapper.selectListByQuery(
                QueryWrapper.create()
                        .eq("tenant_id", tenantId)
                        .eq("is_deleted", 0)
                        ...
                        .orderBy("updated_at", false)
                        .limit(limit)
        ).stream().map(this::toWorkflowItem).toList();
    }
